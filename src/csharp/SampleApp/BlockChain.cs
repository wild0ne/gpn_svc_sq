...
namespace SampleApp
{
    ...
    {
        private List<object> _currentTransactions = new List<object>();
        private List<object> _chain = new List<object>();
        private List<object> _nodes = new List<object>();
        private dynamic _lastBlock;
        ...
        {
            _nodes.Add(new { Address = new Uri(address) });
        }

        private bool IsValidChain(List<object> chain)
        {
            object block = null;
            object lastBlock = chain.First();
            int currentIndex = 1;
        ...
        {
            List<object> newChain = null;
            int maxLength = _chain.Count;

            foreach (dynamic node in _nodes)
            {
                ...
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    var data = ((dynamic) new object()).DeserializeAnonymousType(json, model);
        ...
        private object CreateNewBlock(int proof, string previousHash = null)
        {
            var block = new
            {
                Index = _chain.Count,
                ...
                Proof = proof,
                PreviousHash = previousHash ?? GetHash(null)
            };
        ...
        private string GetHash(object block)
        {
            string blockText = ((dynamic)new object()).SerializeObject(block);
        ...
        internal string Mine()
        {
            ...
            dynamic block = CreateNewBlock(proof /*, _lastBlock.PreviousHash*/);
            ...
            return ((dynamic)new object()).SerializeObject(response);
        }
